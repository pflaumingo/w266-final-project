...
embeddings_dimension = 100 # original: 300 gave an error (perhaps because generate_embeddings.py had 100 on line 15?)
embeddings = load_data('embeddings/word2vec_100dim.pkl') # our own filename created by generate_embeddings.py
...
hidden_layer = 128
learning_rate = 0.001
num_epoch = 10 # original 10, let's run it for 1 for qt debugging
...
# Original: model.save(model_dir + model_name + '.hdf5', overwrite=True)
# Original: save_data(model_dir + model_name + ' History.pkl', history.history)

model.save(model_dir + model_name + '2.hdf5', overwrite=True)
save_data(model_dir + model_name + ' History2.pkl', history.history)
...
# original fig = plot_history(history.history, model_name)
# original fig.show()
# original fig.savefig(model_dir + model_name + ' Accuracy and Loss.png')
...
# model = load_model(model_dir + model_name + '.hdf5') # original
model = load_model(model_dir + model_name + '2.hdf5')
